...
<div style="height: 619px;background-color: #ecf0f5">
    ...
    <form action="${pageContext.request.contextPath}/spacemanager/spaceUpdate">
        <input type="hidden" name="id" value="${carPosition.id}">
            ...
            <div class="box box-primary">
                ...
                <div class="box-body" style="padding-left: 300px">
                    <div style="width: 400px" class="layui-form-item">
                        ...
                        <select class="form-control select2" style="width: 100%" name="carArea.id">
                            <c:forEach items="${carAreaList}" var="carArea">
                                <c:if test="${carPosition.carArea.id == carArea.id}">
                                    <option value="${carArea.id}" selected>${carArea.areaName}</option>
                                </c:if>
                                <c:if test="${carPosition.carArea.id != carArea.id}">
                                    <option value="${carArea.id}">${carArea.areaName}</option>
                                </c:if>
                            </c:forEach>
                        </select>
                        </div>
                    ...
                    <div style="width: 400px" class="layui-form-item">
                        ...
                        <div class="layui-input-block">
                            <input type="text" name="positionNO" required lay-verify="required" placeholder="请输入车位编号"
                                   autocomplete="off" class="layui-input" value="${carPosition.positionNO}">
                        </div>
                    </div>
                    <div style="width: 400px" class="layui-form-item">
                        ...
                        <div class="layui-input-block">
                            <select class="form-control select2" style="width: 100%" name="isactive">
                                <c:if test="${carPosition.isactive == 0}">
                                    <option value="0" selected="selected">空闲</option>
                                    <option value="1">非空闲</option>
                                </c:if>
                                <c:if test="${carPosition.isactive == 1}">
                                    <option value="0">空闲</option>
                                    <option value="1" selected="selected">非空闲</option>
                                </c:if>
                            </select>
                        </div>
                    </div>
